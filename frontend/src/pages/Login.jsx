...
const Login = () => {
  ...
  const [users, setUsers] = useState(() => {
    try { const raw = localStorage.getItem('auth:users'); return raw ? JSON.parse(raw) : []; } catch { return []; }
  });
  const [mode, setMode] = useState('login'); // explicit UI-driven mode
  const mail = email.trim().toLowerCase();
  const existing = useMemo(() => users.find((u) => u.email === mail) || null, [users, mail]);
  ...
  const onSubmit = (e) => {
    ...
    if (mode === 'login') {
      ...
      // Backward compatibility: if user has no password stored, set it now
      if (!existing.password) {
        if (!password) { setError('Please set a password for your account.'); return; }
        const nextUsers = users.map((u) => u.email === mail ? { ...u, password } : u);
        setUsers(nextUsers);
        try { localStorage.setItem('auth:users', JSON.stringify(nextUsers)); } catch {}
        try { localStorage.setItem('auth:user', JSON.stringify({ ...existing, password })); } catch {}
      ...
      try { localStorage.setItem('auth:user', JSON.stringify(existing)); } catch {}
      setUser(existing);
      navigate(redirect || '/', { replace: true });
      return;
    }
    ...
    if (existing) { setError('An account with this email already exists. Please sign in above.'); setMode('login'); return; }
    const newUser = {
      id: mail,
    ...
    const next = [newUser, ...users.filter((u) => u.email !== mail)];
    setUsers(next);
    try { localStorage.setItem('auth:users', JSON.stringify(next)); } catch {}
      ...
      <main className="bg-white">
        <div className="max-w-md mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="mt-6 flex gap-3">
            <button onClick={() => navigate('/orders')} className="rounded-md bg-black px-4 py-2 text-sm font-medium text-white hover:bg-gray-900">Go to Orders</button>
        ...
        {mode === 'login' ? (
          <>
            ...
            <form onSubmit={(e) => { setMode('login'); onSubmit(e); }} className="mt-6 space-y-4">
              ...
              <button type="submit" className="w-full rounded-md bg-black px-4 py-2.5 text-sm font-medium text-white hover:bg-gray-900">Sign in</button>
            </form>
            <a
          ...
          <>
            ...
            <form onSubmit={(e) => { setMode('register'); onSubmit(e); }} className="mt-6 space-y-4">
              ...
              {error && <div className="mt-1 text-sm text-red-600">{error}</div>}
              <button type="submit" className="w-full rounded-md border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-100">Sign up</button>
            </form>
            <a
              href={`${BASE_URL}/api/auth/google?redirect=${encodeURIComponent(redirect)}`}
              className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50"
