...
export default function CustomerDetail() {
  ...
  async function onUpdate(payload) {
    try { await updateCustomerProfile(id, payload); toast.success('Updated'); load(); } catch (e) { toast.error('Update failed'); }
  }

  async function onNotify() {
    if (!emailForm.subject || !emailForm.message) return toast.error('Subject and message required');
    try { await notifyCustomer(id, emailForm); toast.success('Email sent'); setEmailForm({ subject:'', message:'' }); } catch(e){ toast.error('Send failed'); }
  }

  async function onBlacklist() {
    try { await blacklistCustomer(id); toast.success('Blacklisted'); load(); } catch(e){ toast.error('Failed'); }
  }
